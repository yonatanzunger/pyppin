...
class RingBuffer(MutableSequence[T]):
    ...
    def __getitem__(self, index: Union[int, slice]) -> Union[T, List[T]]:
        ...
        print(f'Converted slice {index} to position slices {pos_slices}')
    ...
    def __setitem__(self, index: Union[int, slice], value: Union[T, Iterable[T]]) -> None:
        ...
        if index.start == self._size:
            ...
            count = (
                max(0, (index.stop - start) // (index.step or 1))
        ...
        index = self._normalize_slice(index)
        old_data_length = self._normalized_slice_length(index)
        ...
        new_data_length = len(value) if hasattr(value, '__len__') else None
    ...
    def append(self, value: T) -> None:
        print(f'Append {value} size {self._size} cap {self._capacity} d {self._data}')
        if self._capacity == 0:
        ...
        print(f'Now size {self._size} cap {self._capacity} d {self._data} start {self._start}')
    ...
    def _position(self, index: int, stop: bool = False) -> Tuple[int, bool]:
        ...
        # The overflow rules for start and stop indices are different.
        if stop:
            return (pos, False) if pos <= self._size else (self._safemod(pos, self._size), True)
        else:
            return (pos, False) if pos < self._size else (self._safemod(pos, self._size), True)
    ...
    def _map_slice(self, index: slice) -> Tuple[slice, ...]:
        ...
        print(
            f'_start {self._start} _size {self._size} start {index.start} => {start_pos}, '
            f'{start_wrapped}  stop {index.stop} => {stop_pos}, {stop_wrapped}'
        )

        if stop_wrapped and not start_wrapped:
            # We need two disjoint slices!
            return (slice(start_pos, self._size, index.step), slice(0, stop_pos, index.step))
        else:
    ...
    def _normalize_slice(self, index: slice) -> slice:
        ...
        stop = self._make_nonnegative(index.stop if index.stop is not None else self._size)
        step = index.step or 1
        result = slice(start, stop, step)
        print(f'Normalized {index} to {result}')
        return result

    def _normalized_slice_length(self, normalized: slice) -> int:
        """Return the number of elements in a normalized slice."""
        return max(0, ((normalized.stop - normalized.start) // normalized.step))
    ...
    def _set_at(self, index: int, value: T) -> None:
        ...
        if pos == len(self._data):
            ...
            print(f'Set {value} at position {pos}')
    ...
    def _restructure(self, keep_head: int, data: List[T], keep_tail: int) -> None:
        ...
        print(f'Restructure: size {self._size} [{keep_head}] + [{len(data)}] + [{keep_tail}]')
        ...
        print(f'Pruned: size {self._size} [{keep_head}] + [{len(data)}] + [{keep_tail}]')
        print(f'Head: {self[:keep_head]}')
        print(f'Body: {data}')
        print(f'Tail: {self[-keep_tail:]}')

        new_size = keep_head + keep_tail + len(data)
        assert new_size <= self._capacity
        self._data = self[:keep_head] + data + self[-keep_tail:]
        self._size = new_size
        self._start = 0
